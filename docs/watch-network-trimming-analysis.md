...
其中，“服务折叠”是指保持逻辑服务边界不变，但根据产品画像将多个逻辑 SA 部署到同一物理进程域，以减少进程和 IPC 开销。
...
HTTP/TLS 尽量作为按需库进入调用进程，统计、诊断和扩展服务按需拉起或完全移除。手机产品仍可保留多 SA 隔离；手表则由部署画像执行服务折叠，从而减少进程和 IPC 开销并避免维护独立分支。
