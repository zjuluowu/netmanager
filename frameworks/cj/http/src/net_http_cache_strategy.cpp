...
HttpCacheStrategy::HttpCacheStrategy(HttpRequest &requestOptions) : requestOptions_(requestOptions)
{
...
{
    return requestOptions_.GetMethod() == HTTP_METHOD_GET ||
           requestOptions_.GetMethod() == HTTP_METHOD_HEAD ||
...
int64_t HttpCacheStrategy::CacheResponseAgeMillis()
{
    ...
    the "apparent_age": response_time minus date_value,
    if the local clock is reasonably well synchronized to the origin
    ...
    int64_t nowTime = std::max<int64_t>(0, GetNowTimeSeconds());
