...
HWTEST_F(NetConnClientTest, GetIfaceNameIdentMapsTest001, TestSize.Level1)
{
    ...
    int32_t ret =
        DelayedSingleton<NetConnClient>::GetInstance()->GetIfaceNameIdentMaps(NetBearType::BEARER_CELLULAR, data);
    EXPECT_EQ(ret, NETMANAGER_SUCCESS);
}
...
HWTEST_F(NetConnClientTest, DecreaseSupplierScore001, TestSize.Level1)
{
    ...
    uint32_t supplierId = 100;
    int32_t ret =
        NetConnClient::GetInstance().DecreaseSupplierScore(NetBearType::BEARER_WIFI, "test", supplierId);
